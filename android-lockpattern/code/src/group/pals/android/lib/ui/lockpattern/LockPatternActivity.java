...
public class LockPatternActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH)
			theme = getIntent().getIntExtra(_Theme,
	...
	private final View.OnClickListener mBtnConfirmOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			if (getString(R.string.alp_cmd_continue).equals(
				...
				Intent i = new Intent();
				// i.putExtra(_Pattern,
				// LockPatternUtils.patternToString(mLastPattern));

				String patternStr = "";
				final int patternSize = mLastPattern.size();
				for (int j = 0; j < patternSize; j++) {
					LockPatternView.Cell cell = mLastPattern.get(j);
					int pos = cell.getColumn() + 3 * cell.getRow();
					patternStr = patternStr + pos;
				}
				i.putExtra(_Pattern, patternStr);
